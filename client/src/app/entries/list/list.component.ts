      ...
      <ng-container *ngIf="searchService.results$ | async as searchState">
        ...
        <ng-container *ngIf="!searchState.active">
          ...
          <div class="timeline-scroller">
            <button mat-button class="timeline-nav-button" (click)="jumpToFirstEntry()" [disabled]="!hasEntries()">
              First
            </button>
            
            <button mat-icon-button (click)="scrollTimeline(-1)" [disabled]="timelineScrollIndex <= minScrollIndex">
            ...
            <div class="timeline-months">
              ...
              >
                ...
                <div class="entry-count-badge" *ngIf="item.entryCount && !item.isFuture">
                  {{ item.entryCount }}
                </div>
              </div>
            </div>
            
            <button mat-icon-button (click)="scrollTimeline(1)" [disabled]="timelineScrollIndex >= maxScrollIndex">
              <mat-icon>chevron_right</mat-icon>
            </button>

            <button mat-button class="timeline-nav-button" (click)="jumpToToday()">
              Today
            </button>
          </div>
          ...
          <div class="no-entries-message" *ngIf="paginatedEntries.length === 0">
            <mat-card class="no-entries-card">
              <mat-card-content>
                ...
                <p>No entries for this time period. Start documenting your journey!</p>
                <button mat-raised-button color="primary" (click)="navigateToCreateEntry()">
...
export class ListComponent implements OnInit, OnDestroy {
  ...
  generateDynamicTimeline(entries: any[]): TimelineMonth[] {
    ...
    while (current <= endDate) {
      const isCurrentMonth = current.getMonth() === now.getMonth() && 
    ...
    if (this.selectedMonth) {
      ...
      const existingSelectedMonth = months.find(m => 
  ...
  centerTimelineOnCurrentMonth(): void {
    if (this.selectedMonth) {
      const selectedIndex = this.allMonths.findIndex(m => m === this.selectedMonth);
  ...
  centerTimelineAnimated(targetIndex: number): void {
    const targetScrollIndex = Math.max(0, Math.min(targetIndex - 2, this.maxScrollIndex));
  ...
  updateScrollLimits(): void {
    ...
    const allEntries = [...this.dailyEntries, ...this.dreamEntries];
    if (allEntries.length > 0) {
      const earliestDate = new Date(Math.min(...allEntries.map(e => new Date(e.entry_date).getTime())));
      const earliestMonthIndex = this.allMonths.findIndex(m => 
  ...
  updateVisibleMonths(): void {
    this.visibleMonths = this.allMonths.slice(this.timelineScrollIndex, this.timelineScrollIndex + 5);
  }
  ...
  selectMonth(month: TimelineMonth): void {
    ...
    if (month.isFuture) return; // Don't allow selection of future months
    ...
    const selectedVisibleIndex = this.visibleMonths.findIndex(m => m === month);
  ...
  onViewChange(view: string): void {
    ...
    // Simply filter entries for current selected date/month without jumping timeline
    this.filterEntries();
    
    // Reset pagination to first page since results may have changed
    this.currentPage = 0;
    this.updatePaginatedEntries();
  ...
  resetToCurrentMonth(): void {
    ...
    if (currentMonth) {
      ...
      const currentIndex = this.allMonths.findIndex(m => m.isCurrent);
      this.timelineScrollIndex = Math.max(0, Math.min(currentIndex - 2, this.maxScrollIndex));
      this.updateVisibleMonths();
    }
  }
  ...
  jumpToFirstEntry(): void {
    ...
    const earliestEntry = allEntries.reduce((earliest, entry) => 
      new Date(entry.entry_date) < new Date(earliest.entry_date) ? entry : earliest
    );
    const earliestDate = new Date(earliestEntry.entry_date);
    
    // Find corresponding month in timeline
    const earliestMonth = this.allMonths.find(m => 
      m.year === earliestDate.getFullYear() && 
      (m as any).monthIndex === earliestDate.getMonth()
    );
    ...
    if (earliestMonth) {
      ...
      const earliestIndex = this.allMonths.findIndex(m => m === earliestMonth);
  ...
  filterEntries(): void {
    ...
    if (this.selectedMonth) {
      entries = entries.filter(entry => {
        ...
        return entryDate.getMonth() === (this.selectedMonth as any).monthIndex &&
               entryDate.getFullYear() === this.selectedMonth!.year;
      });
    }
    
    // Sort by date (newest first)
    this.filteredEntries = entries.sort((a, b) => 
  ...
  calculateEntryCountsForTimeline(): void {
    // Get all entries for counting (regardless of current view)
    const allEntries = [...this.dailyEntries, ...this.dreamEntries];
    
    // Update each timeline month with entry count
    this.allMonths.forEach(month => {
      const count = allEntries.filter(entry => {
        const entryDate = new Date(entry.entry_date);
        return entryDate.getMonth() === (month as any).monthIndex &&
               entryDate.getFullYear() === month.year;
      }).length;
      
      month.entryCount = count > 0 ? count : undefined; // Only set if > 0
    });
