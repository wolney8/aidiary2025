    ...
    <div class="import-container" role="main" aria-label="Import entries">
      ...
      <mat-card class="step-card">
        <mat-card-header>
          ...
          <mat-card-subtitle>
            Get the Excel template, fill it with your entries, then upload below.
          </mat-card-subtitle>
        </mat-card-header>
        <mat-card-content>
          <p class="hint-text">
            The template contains separate sheets for <strong>Daily</strong> and
            <strong>Dream</strong> entries with all required columns pre-defined.
          </p>
        </mat-card-content>
        <mat-card-actions>
          <button
      ...
      <mat-card class="step-card">
        ...
        <mat-card-content>
          ...
          >
            ...
            <ng-container *ngIf="!selectedFile; else fileSelected">
              ...
              <p class="drop-secondary">Accepts .xlsx and .xls — maximum 5 MB</p>
            </ng-container>
          ...
          >
            ...
            <div class="feedback-body">
              ...
              <ul *ngIf="importResult!.errors && importResult!.errors!.length" class="error-list">
        ...
        <mat-card-actions class="upload-actions">
          ...
          <button
            ...
            color="primary"
            (click)="uploadFile()"
            [disabled]="!selectedFile || !!validationError || uploadState === 'uploading'"
      ...
      <mat-card class="step-card">
        <mat-card-header>
          ...
          <mat-card-subtitle>Recent import sessions for your account.</mat-card-subtitle>
        </mat-card-header>

        <mat-card-content>

          <!-- Loading -->
          <div *ngIf="historyLoading" class="history-loading" role="status" aria-live="polite">
            <mat-progress-bar mode="indeterminate" aria-label="Loading import history"></mat-progress-bar>
          ...
          <!-- History table -->
          <div *ngIf="!historyLoading && !historyError && history.length > 0" class="table-wrapper">
            <table
              mat-table
              [dataSource]="history"
            ...
            >
              ...
              <ng-container matColumnDef="imported_at">
                ...
                <td mat-cell *matCellDef="let row">{{ formatDate(row.imported_at) }}</td>
              </ng-container>
              ...
              <ng-container matColumnDef="status">
                ...
                <td mat-cell *matCellDef="let row">
                  <mat-chip
                    ...
                    disabled
                  >
                    <mat-icon class="chip-icon" aria-hidden="true">{{ statusIcon(row.status) }}</mat-icon>
...
export class ImportComponent implements OnInit {
  ...
  readonly historyColumns = ['imported_at', 'filename', 'imported_count', 'skipped_count', 'error_count', 'status'];
  ...
  uploadFile(): void {
    ...
    this.uploadProgress = { percent: 0, loaded: 0, total: this.selectedFile.size };
    this.importResult = null;
    this.importErrorMessage = '';

    this.importService.uploadFile(this.selectedFile).pipe(
      ...
      next: event => {
        ...
        if (event.type === 'progress') {
          ...
          if (event.result.status === 'failed') {
            ...
            this.importErrorMessage = event.result.message || 'Import failed.';
          } else {
            this.uploadState = event.result.status;
          }
          if (this.uploadState === 'success' || this.uploadState === 'partial') {
            this.loadHistory();
          }
        }
      },
      error: (err: Error) => {
        this.uploadState = 'error';
        this.importErrorMessage = err.message || 'Upload failed. Please try again.';
      }
    });
  }
