...
def _write_cutover_artifacts(tmp_path, *, ready=True, baseline_errors=0):
    ...
    _write_json(
        preflight_report,
        {
            ...
            "blockers": [] if ready else [{"gate": "jwt_secret"}],
            "warnings": [],
        },
    )
    _write_json(
...
def test_cutover_evidence_packet_reports_missing_evidence_flags(tmp_path):
    ...
    assert "preflight_report_ready_for_production" in packet["blockers"]
...
def test_cutover_evidence_packet_requires_preflight_report_for_completion(tmp_path):
    (
        sqlite_backup,
        export_dir,
        migration_report,
    ...
    assert packet["packet_complete"] is False
    assert "preflight_report_missing" in packet["blockers"]
