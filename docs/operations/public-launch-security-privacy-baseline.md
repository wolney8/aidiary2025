...
| Major | Billing and entitlements | Public SaaS requires a payment provider, local entitlement model, subscription lifecycle handling, upgrade prompts, and billing disclosures. | Entitlement tables, Stripe Checkout, Customer Portal session creation, and verified webhook entitlement sync exist. Plan gates, production preflight checks, and billing disclosures remain. | `#130`, `#131`, `#132` |
