...
| `cd client && npm run test:e2e:a11y` | Passed, 29 axe/reflow/keyboard checks across public/authenticated routes, entry filters, entry creation, create-entry AI/attachment state, populated entry detail, On This Day previews, populated and expanded search, Profile, Settings/Appearance/Export/Customisation, standalone Important Days and Important Day editing, Thought Record dashboard and worksheet, Reflection summaries, notification overlays, monthly preview decks, import review modal, transcript/derived-text dialog, compact shell keyboard behavior, and light/dark representative routes |
