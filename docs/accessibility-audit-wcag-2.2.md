...
| 1.4.12 Text Spacing | Partial automated pass | Notification, calendar preview, image, import-review, transcript/derived-text overlays, populated search results, populated entry detail, Customisation settings, standalone Important Day editing, and create-entry AI/attachment form state are covered by the WCAG text-spacing gate; complete manual route smoke at 200% zoom. |
...
| `cd client && npm run test:e2e:a11y` | Passed, 24 axe/reflow/keyboard checks across public/authenticated routes, entry filters, entry creation, create-entry AI/attachment state, populated entry detail, On This Day previews, populated and expanded search, Settings/Customisation, standalone Important Days and Important Day editing, notification overlays, monthly preview decks, import review modal, transcript/derived-text dialog, compact shell keyboard behavior, and light/dark representative routes |
